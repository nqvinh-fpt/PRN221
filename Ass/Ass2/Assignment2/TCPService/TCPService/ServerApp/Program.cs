    ...
    static void ProcessMessage(object parm)
{
    ...
    try
    {
        ...
        while ((count = stream.Read(bytes, 0, bytes.Length)) != 0)
        {
            ...
            data = Encoding.UTF8.GetString(bytes, 0, count); // Use UTF-8 decoding
            Console.WriteLine($"Received: {data} at {DateTime.Now:t}");
            // Process the data sent by the client.
            byte[] msg = Encoding.UTF8.GetBytes(data); // Use UTF-8 encoding
            // Send back a response without converting to uppercase.
